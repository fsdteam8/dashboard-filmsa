...
export function VideoUpload({
  ...
  const uploadSingleChunk = async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `❌ HTTP error for chunk ${chunkNumber}: ${response.status} - ${errorText}`
        );
        throw new Error(
  ...
  const uploadChunkWithRetry = async (
    ...
    while (attempts < MAX_RETRIES) {
      ...
      try {
        ...
        if (success) {
          ...
          console.log(
            `🎉 Chunk ${chunkNumber} completed successfully on attempt ${attempts}`
          );
          return;
        }
      } catch (error) {
        if (error instanceof Error && error.name === "AbortError") {
          console.log(
            `🛑 Chunk ${chunkNumber} upload aborted during attempt ${attempts}`
          );
          throw error;
        }
        console.error(
          `💥 Chunk ${chunkNumber} upload attempt ${attempts} failed:`,
          error
        );
      }
      ...
      if (attempts < MAX_RETRIES) {
        ...
        console.log(
          `⏳ Waiting ${delay}ms before retry for chunk ${chunkNumber}`
        );
    ...
    throw new Error(
      `Failed to upload chunk ${chunkNumber} after ${MAX_RETRIES} attempts`
    );
  };
  ...
  const uploadFileInChunks = async (file: File): Promise<ChunkUploadResult> => {
    ...
    try {
      ...
      for (let i = 0; i < chunks; i++) {
        ...
        console.log(
          `📈 Progress: ${progress}% (${i + 1}/${chunks} chunks completed)`
        );
      }
      ...
      console.log(
        `📊 Average chunk upload time: ${Math.round(
          chunksUploadTime / chunks
        )}ms`
      );
