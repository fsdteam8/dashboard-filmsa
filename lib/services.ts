...
export const genreService = {
  getGenres: async (page = 1): Promise<{ data: Genre[]; total: number; per_page: number; current_page: number }> => {
...
export const contentService = {
  getContents: async (
    page = 1,
  ): Promise<{ data: Content[]; total: number; per_page: number; current_page: number }> => {
    const response = await api.get(`/api/contents?page=${page}`)
    return response.data
  },

  createContent: async (data: FormData): Promise<Content> => {
...
export const subscriptionService = {
  getSubscriptions: async (): Promise<{ data: Subscription[]; withads_count: number; withoutads_count: number }> => {
  ...
  updateSubscription: async (
    ...
    const response = await api.post(`/api/subscriptions/${id}?_method=PUT`, data)
    return response.data
  },
