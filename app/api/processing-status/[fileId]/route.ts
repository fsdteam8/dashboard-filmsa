...
export async function GET(request: NextRequest, { params }: { params: { fileId: string } }) {
  try {
    ...
    const originalFiles = originalListResult.Contents?.filter((file) => file.Key?.includes(fileId)) || []
    ...
    const status = {
      fileId,
      processing: {
        status: playlistFile ? "completed" : hlsFiles.length > 0 ? "in_progress" : "pending",
