  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 px-6 py-4 w-full h-[84px]">
      <div className="flex items-center justify-between w-full">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-3 text-gray-900 hover:text-gray-600 focus:outline-none bg-[#272727]">
          ...
          <DropdownMenuContent align="end" className="bg-white border-gray-200 w-64">
            <div className="px-4 py-3 border-b border-gray-200">
              <div className="font-medium text-gray-900">{session?.user?.email}</div>
              <div className="text-sm text-gray-500 capitalize">{session?.user?.role}</div>
            </div>
